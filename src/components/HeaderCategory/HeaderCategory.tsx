...
import { selectCountries, selectGenres, setCountries, setCountriesList, setGenres, setGenresList, setPickedYear } from '@/src/store/reducers/filterReducer'
...
const HeaderCategory = () => {
  ...
  useEffect(() => {
    MoviesService.getGenres()
      .then((json: IGenre[]) => setAllGenres(json.sort(sortData('id'))))
      .catch((err: any) => console.log(err))
    MoviesService.getCountries()
      .then((json: IGenre[]) => setCountry(json.sort(sortData('id'))))
      .catch((err: any) => console.log(err))
  }, [])
    ...
    <div className={styles.headerContent}>
      <div className={`${styles.list} ${styles.border}`}>
        ...
        {allGenres.map((el, idx) =>
          idx < 18 ? (
            <Link
              ...
              href='/movies'
            >
              <span>{capitalize(el.nameRu)}</span>
            </Link>
          ) : (
            ''
          )
        ...
        {countryArray.map((el, idx) =>
          idx < 16 ? (
            <Link key={idx} className={styles.listFont} onClick={() => addCountry(el.id, el.nameRu)} href='/movies'>
              <span>{el.nameRu}</span>
            </Link>
          ) : (
            ''
      ...
      <div className={`${styles.list} ${styles.list__secondBlock}`}>
        {headerDataArray.map((el, idx) => (
          <Link key={idx} href={el.link} className={styles.listFont}>
            <span>{t(el.content)}</span>
          </Link>
        ))}
      </div>
    </div>
