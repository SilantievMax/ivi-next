...
const HomePage: FC = () => {
  ...
  useEffect(() => {
    fetch(
      `http://localhost:3003/info?limit=10`,
      {
        method: 'GET'
      }
    )
    ...
    fetch(
      `http://localhost:3003/info?limit=20&genres=2`,
      {
        method: 'GET'
      }
    )
    ...
    fetch(
      `http://localhost:3003/info?limit=20&genres=4`,
      {
        method: 'GET'
      }
    )
    ...
    <div>
      ...
      <Slider {...settings}>
        {mainPageSliderData.map((el, idx) =>
          <div key={idx} className={styles.sliderItemContainer}>
            ...
            <Button className={styles.sliderButton} color='red'>{t('watchFree')}</Button>
          </div>)}
      </Slider>
      <div className={styles.btnContainer}>
        <Button img='https://solea-parent.dfs.ivi.ru/picture/ffffff,ffffff/lightning.svg'
                color='mainBlueGradient' className={`${styles.btn} ${styles.btn__img}`}>{t('subscribe30')}</Button>
        <Button img='https://solea-parent.dfs.ivi.ru/picture/ffffff,ffffff/gift.svg'
                className={styles.btn} color='mainGray'>{t('promo-code')}</Button>
      </div>
      <div className={styles.mainSection__description}>
        ...
        <div className={filmsStyles.descriptionWrapper}>
          <p
            className={showDescription ? `${styles.descriptionFont} ${filmsStyles.greyText}` : `${styles.descriptionFont} ${filmsStyles.greyText} ${filmsStyles.hiddenText}`}>
            {t('mainContentP1')}
          </p>
          <p
            className={showDescription ? `${styles.descriptionFont} ${filmsStyles.greyText}` : `${styles.descriptionFont} ${filmsStyles.greyText} ${filmsStyles.none}`}>
            {t('mainContentP2')}
          </p>
          <p
            className={showDescription ? `${styles.descriptionFont} ${filmsStyles.greyText}` : `${filmsStyles.descriptionFont} ${filmsStyles.greyText} ${filmsStyles.none}`}>
            {t('mainContentP3')}
          </p>
          <ol
            className={showDescription ? `${styles.descriptionFont} ${filmsStyles.greyText} ${styles.list}` : `${filmsStyles.none}`}>
            {contentList.map((el, idx) => <li className={styles.listElem} key={idx}>
        ...
        <span className={filmsStyles.clauseToggle}
